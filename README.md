...
- ✅ v0.5.0 — scroller app v1 (2026-06-07): dedicated `/amazon`, `/wikivoyage`, `/images` routes; screenshot pipeline (`scripts/capture-screenshots.mjs` → `s3://com27/scroller/screenshots/<id>.png`); prompt heroes (`scripts/generate-prompt-images.mjs` → `s3://com27/scroller/prompts/<slug>.png`); global view-persistence with default = scroller; dual-channel YouTube (handle resolver merges `@MatSiems` + `@mat-siems-production`); GitHub OG-image + owner avatar enrichment; per-prompt Copy button; Mongo wiki cache (522+ articles in `AIDB.wiki` — 230 random Wikipedia + 175 WikiVoyage + 117 cluster-seeded). `listWikiIndex`/`listWikiCategories` fall back to Mongo when Supabase index is empty so `/wiki` reports the actual count without the migration applied. com27 bucket policy gained `PublicReadScroller` Allow for `scroller/*`.
...
- 📋 Apply `supabase/migrations/0002_0003_wiki_combined.sql` to Supabase Studio (single self-contained 165-line paste, idempotent — supersedes the 0001/0002/0003 sequence). Required for the daily cron at `0 4 * * *` to write to `scroller_wiki_index`. Mongo fallback covers reads in the meantime.
- 📋 Deploy v0.5.0 to scroller-psi (matsiems-scope serves scroller-bay/`scroller-matsiems-matsiems.vercel.app`; psi alias is on the old cleverfox-71aa03f5 scope and requires `vercel login` as mat-flexappdev)
- 📋 v0.6.0 — Playwright E2E, ESLint CLI migration, Vercel Analytics, Sentry; replace prompt placeholder PNGs with `/iad`-generated hero images per prompt
