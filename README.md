...
- ✅ v0.6.0 — scroller app v2 (2026-06-10): desktop side-panel preview (lg+ opens right panel, grid keeps visible), full 15-field S3 image metadata (HeadObjectCommand for per-item detail), wiki page converted to PageBrowser (same scroller/list/grid views as all other pages), WikiVoyage 67,856-article bulk cache via `generator=allpages`. Architecture diagram (Cleverfox editorial HTML+SVG, `/about` iframe embed + `public/diagrams/architecture.html`). Runware FLUX OG hero image (`public/og-hero.png`, 1024×576, wired into layout OG + Twitter card). Runware prompt-hero batch script (`scripts/generate-prompt-images-runware.mjs`) — replaces SVG-gradient PNGs with AI-generated heroes; run with `node scripts/generate-prompt-images-runware.mjs --skip-existing`.
...
- 📋 Apply `supabase/migrations/0002_0003_wiki_combined.sql` to Supabase Studio (idempotent). Required for the daily wiki cron. Mongo fallback covers reads in the meantime.
